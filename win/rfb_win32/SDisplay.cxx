...
EnumParameter rfb::win32::SDisplay::disconnectAction("DisconnectAction",
  core::format("%s (%s)",
    _("Action to perform when all clients have disconnected"),
    "None, Lock, Logoff").c_str(),
...
void SDisplay::layoutRequest(int, int, ScreenSet)
{
  assert(server != nullptr);
...
SDisplay::processEvent(HANDLE event) {
  if (event == updateEvent) {
    ...
    if (!core) {
      vlog.error(_("Desktop is not started"));
      return;
    }
