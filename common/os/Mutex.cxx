...
Mutex::Mutex()
{
  ...
  if (ret != 0)
    throw core::SystemException("Failed to create mutex", ret);
#endif
}
...
void Mutex::lock()
{
  ...
  if (ret != 0)
    throw core::SystemException("Failed to lock mutex", ret);
#endif
}
...
void Mutex::unlock()
{
  ...
  if (ret != 0)
    throw core::SystemException("Failed to unlock mutex", ret);
#endif
}
...
Condition::Condition(Mutex* mutex_)
{
  ...
  if (ret != 0)
    throw core::SystemException("Failed to create condition variable", ret);
#endif
}
...
void Condition::wait()
{
  ...
  ret = SleepConditionVariableCS((CONDITION_VARIABLE*)systemCondition,
                                 ...
                                 INFINITE);
  if (!ret)
    throw core::SystemException("Failed to wait on condition variable", GetLastError());
#else
  int ret;
  ...
  if (ret != 0)
    throw core::SystemException("Failed to wait on condition variable", ret);
#endif
}
...
void Condition::signal()
{
  ...
  if (ret != 0)
    throw core::SystemException("Failed to signal condition variable", ret);
#endif
}
...
void Condition::broadcast()
{
  ...
  if (ret != 0)
    throw core::SystemException("Failed to broadcast condition variable", ret);
#endif
}
