...
void ZlibInStream::flushUnderlying()
{
  while (bytesIn > 0) {
    if (!hasData(1))
      throw core::Exception("ZlibInStream: failed to flush remaining stream data");
    skip(avail());
  }
...
void ZlibInStream::init()
{
  ...
  if (inflateInit(zs) != Z_OK) {
    delete zs;
    zs = nullptr;
    throw core::Exception("ZlibInStream: inflateInit failed");
  }
}
...
{
  if (!underlying)
    throw core::Exception("ZlibInStream overrun: no underlying stream");
  ...
  if (rc < 0) {
    throw core::Exception("ZlibInStream: inflate failed");
  }
