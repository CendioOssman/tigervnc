...
GestureHandler::GestureHandler() :
  ...
  longpressTimer(new QTimer), twoTouchTimer(new QTimer), touchHandler(new X11TouchHandler(this))
{
  connect(longpressTimer, &QTimer::timeout, this, &GestureHandler::longpressTimeout);
  longpressTimer->setSingleShot(true);
  longpressTimer->setInterval(GH_LONGPRESS_TIMEOUT);

  connect(twoTouchTimer, &QTimer::timeout, this, &GestureHandler::twoTouchTimeout);
  twoTouchTimer->setSingleShot(true);
  twoTouchTimer->setInterval(GH_TWOTOUCH_TIMEOUT);
}

GestureHandler::~GestureHandler()
{
  delete longpressTimer;
  delete twoTouchTimer;
  delete touchHandler;
}
...
void GestureHandler::handleTouchUpdate(int id, double x, double y)
{
  ...
  if (!hasDetectedGesture()) {
    ...
    if (tracked.size() == 2) {
      ...
      } else if (!twoTouchTimer->isActive()) {
...
void GestureHandler::handleTouchEnd(int id)
{
  ...
  if (!hasDetectedGesture() && twoTouchTimer->isActive()) {
    twoTouchTimer->stop();
    state = GH_NOGESTURE;
  }
...
void GestureHandler::pushEvent(GestureEventType t)
{
  ...
  touchHandler->handleGestureEvent(gev);
}
